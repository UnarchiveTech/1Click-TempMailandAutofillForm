...
import { detectIconFromMessage, ICON_KEYWORD_MAPPING } from './iconMapping';
...
describe('detectIconFromMessage', () => {
  ...
  test('returns "edit" for undo messages (matches "undo" keyword in edit group)', () => {
    // "edit" keywords come before "undo" in the mapping, but "undo" matches first
    // Actually, let's test what actually matches
    expect(detectIconFromMessage('Undo changes')).toBe('edit');
  });
