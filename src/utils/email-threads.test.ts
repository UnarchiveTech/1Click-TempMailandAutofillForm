...
import { groupEmailsByThread, normalizeSubject } from './email-threads';
...
describe('normalizeSubject', () => {
  ...
  test('strips only the first reply prefix', () => {
    // normalizeSubject uses a single regex replace, so it only strips the first prefix
    expect(normalizeSubject('Re: Re: Re: Hello')).toBe('re: re: hello');
  });

  test('strips mixed case prefix (first only)', () => {
...
describe('groupEmailsByThread', () => {
  ...
  test('emails with same subject but different senders and far apart are separate threads', () => {
    ...
    const threads = groupEmailsByThread(emails);
    // The emails are within 72 hours (27.8h < 72h), so they ARE grouped
    // even with different senders. This is the expected behavior.
    expect(threads.length).toBe(1);
  });
  ...
  test('collects unique participants', () => {
    const emails = [
      makeEmail({ id: '1', subject: 'Hello', from: 'a@b.com', received_at: 1700000000 }),
      makeEmail({ id: '2', subject: 'Hello', from: 'c@d.com', received_at: 1700001000 }),
      makeEmail({ id: '3', subject: 'Hello', from: 'a@b.com', received_at: 1700002000 }),
    ];
    const threads = groupEmailsByThread(emails);
    expect(threads[0].participants).toContain('a@b.com');
    expect(threads[0].participants).toContain('c@d.com');
